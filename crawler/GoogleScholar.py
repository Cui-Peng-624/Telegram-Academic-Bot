...
def fetch_google_scholar_results(inquiry, as_ylo, start, hl="zh-CN"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            retry_after = int(response.headers.get("Retry-After", 10)) # 如果没有Retry-After字段的值，则默认使用10
            print(f"Too many requests. Retrying after {retry_after} seconds...")
        else:
            print("Error", response.status_code)
    except requests.exceptions.RequestException as e:
        print(f"Error occurred while fetching Google Scholar results: {e}")
...
def check_and_initialize_persist_file(persist_file):
    if not os.path.exists(persist_file):  # If the persistence file does not exist, create an empty DataFrame and save it
        df_persist = pd.DataFrame(columns=['Title', 'href', 'basic_info', 'Abstract', 'full_abstract'])
...
def fetch_and_save_results_temp(temp_save_path, q, as_ylo, start):
    ...
    if all_results:
        ...
        # 检查 'Title' 列是否存在，并且是否至少有一个非空值
        if 'Title' in df_new.columns and df_new['Title'].notna().any():
            df_new = df_new.dropna(subset=['Title'])  # 删除 'Title' 列中缺失的行
            df_new.to_csv(temp_save_path, index=False)  # 覆写临时文件
            print(f"成功保存{len(df_new)}条结果到{temp_save_path}")
            return df_new
        else:
            print("Warning: No valid 'Title' found in the results. Skipping save.")
            return None
    else:
        print("没有爬取到任何结果！")
        return None
